...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
    std::array<GPIO_TypeDef*, 4> ports = { D1_GPIO_Port, D2_GPIO_Port,
            D3_GPIO_Port, D4_GPIO_Port };
    std::array<uint16_t, 4> pins = { D1_Pin, D2_Pin, D3_Pin, D4_Pin };
    StepMotor28BYJ48 motor(ports, pins);
    motor.beginRotateLeft8();
  /* USER CODE END 2 */
